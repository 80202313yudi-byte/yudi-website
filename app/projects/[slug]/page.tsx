  ...
  return (
    <article className="mx-auto w-full max-w-275 px-6 pt-40 pb-10 sm:px-10 sm:pt-48 sm:pb-14 lg:pt-56">
      <FadeIn className="flex flex-col gap-8">
        <Link
          href="/projects"
          className="focus-ring group inline-flex w-fit items-center gap-2 rounded-xl border border-foreground/8 bg-background px-4 py-2 text-sm font-medium text-foreground/70 transition-colors hover:text-foreground"
        >
          <ArrowLeft
        ...
        <ProjectImageFrame image={project.heroImage} priority large />
      </FadeIn>
    </article>
  );
}
      ...
      <div className="grid gap-8 lg:grid-cols-[0.45fr_1fr] lg:gap-10">
        <FadeIn className="flex flex-col gap-5 rounded-3xl border border-foreground/8 bg-background p-5 shadow-sm sm:p-6">
          ...
          {project.deliverables?.length ? (
            <ul className="mt-auto flex flex-wrap gap-2 border-t border-foreground/8 pt-5">
              {project.deliverables.map((item) => (
                <li
                  key={item}
                  className="rounded-full border border-foreground/8 bg-foreground/[0.03] px-3 py-1.5 text-sm tracking-tight text-foreground/68"
                >
                  {item}
                </li>
              ))}
...
function ProjectMetaItem({
  ...
  return (
    <div className="flex gap-3 rounded-2xl border border-foreground/6 bg-foreground/[0.025] p-3">
      <span className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-foreground/8 bg-background">
        <Icon className="h-4 w-4 text-foreground/70" aria-hidden="true" />
      </span>
      <div className="min-w-0">
        <dt className="text-[12px] tracking-tight text-foreground/45">
          {label}
        </dt>
...
function ProjectPagerLink({
  ...
  return (
    ...
    >
      <span className="inline-flex items-center gap-2 text-sm font-medium tracking-tight text-foreground/55">
        {isPrevious ? (
          <ArrowLeft
            className="h-4 w-4 transition-transform duration-300 group-hover:-translate-x-0.5"
            aria-hidden="true"
